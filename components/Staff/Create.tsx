...
const Create = () => {
    ...
    const [response, setResponse] = useState<Admin>()
    ...
    const onSubmit: SubmitHandler<Staff> = async (data) => {
        ...
        await makeApiRequest("/addAdmin", "post", formData, {
            onSuccess: (response) => {
                toast.success(`The Admin ${response.data.name} was created successfully.`);
                reset();
    ...
    return (
        <>
            {show && <NewDetails name={response?.name!} email={response?.email!} encryptedPassword={response?.encryptedPassword!} toggleFunction={toggleShow}/>}
